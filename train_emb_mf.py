...
def run_get_mask_emb(g, args, imp_num, rewind_weight_mask, item_min, item_max, user_seq, edge_index, train_loader,
    ...
    if rewind_weight_mask is not None:
        model.load_state_dict(rewind_weight_mask)  #rewind
        print("-" * 100)
...
if __name__ == '__main__':
    # logger = get_logger('path', 'name.log')
    ...
    if len(files) != 0:    # if ==2 means start from 2
        if len(files) > 1:
            ...
            if args.dataset == "yelp2018":
                model = torch.load(
                    "./mflot/ckpt/emb/part-join-reg-0.0001-loss-bpr-stop-100-epoch-1000-nonstop-dim-32-per-0.1-imp-1")
